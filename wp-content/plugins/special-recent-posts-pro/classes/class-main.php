...
*/
	...
	// Declaring post author ID (when in single post/page mode).
	private $authorID;
...
*/
	/*
	| -----------------------------------------------------------------------
	| This is the main method for the image manipulation. Every fetched
	| image is stored in the cache folder then displayed on screen.
	...
	*/
	private function getFirstImageUrl($post, $thumb_width, $thumb_height, $post_title) {
		...
		if (!empty($output)) {
			...
			// Image has been found. Determine which image should be extracted based upon the post thumbnail index option.
			if ($this->widget_args['post_thumbnail_index_adv'] != '') {
		...
		if ($this->widget_args["thumbnail_custom_field"] != "") {
			...
			if (!empty($thumb_postmeta)) {
				...
				if ('yes' == $this->widget_args['thumbnail_link']) {
					...
					$thumbimg  = '<img src="' . $thumb_postmeta[0] . '" class="srp-widget-thmb" width="' . $this->widget_args['thumbnail_width'] . '" height="' . $this->widget_args['thumbnail_height'] . '" alt="' . $post->post_title . '" />';
					
					// Building HTML link atts.
					$linkatts = array('class' => 'srp-widget-thmblink', 'href' => get_permalink($post->ID), 'title' => $post->post_title);

					// Checking if the "target='_blank'" option is enabled.
					if ($this->widget_args['targetblank_links']  == 'yes') $linkatts['target'] = '_blank';
					...
					$thumb = '<img src="' . $thumb_postmeta[0] . '" class="srp-widget-thmb" width="' . $this->widget_args['thumbnail_width'] . '" height="' . $this->widget_args['thumbnail_height'] . '" alt="' . $post->post_title . '" />';
	...
	*/
	private function getPosts() {
		...
		if ( ($this->widget_args["category_autofilter"] == 'yes') && (is_category()) ) {
		...
		} else if ( ($this->widget_args["category_autofilter_single"] == 'yes') && is_single()) {
			...
			// Filtering according to the current viewd category page.
			$args["category"] = implode(',', $currentPostCategory);
		...
		if (!empty($this->widget_args["post_exclude"])) {
			...
			$args["exclude"] = ($args["exclude"] != '') ? $args["exclude"] . ', ' . $this->widget_args["post_exclude"] : $this->widget_args["post_exclude"];
		}
		...
		// Checking if tags filtering is on.
		if ($this->widget_args["author_autofilter"] == 'yes') {
	...
	*/
	public function displayPosts($widget_call = NULL, $return_mode) {
		...
		// Defining list of post thumbnails.
		$postThumbHtml = array();
		...
		if ('yes' != $this->widget_args["widget_title_hide"]) {
			...
			if ($this->widget_args["category_title"] == "yes") {
				...
				// Building HTML link atts.
				$linkatts = array('class' => 'srp-widget-title-link', 'href' => $srp_category_link, 'title' => get_cat_name($thisCategoryId));
				...
				if ($this->widget_args["widget_title_link"] != "") {
					...
					switch ($this->widget_args["post_status"]) {
						...
						case "trash":
							...
							$widgetTitleLink = (strpos($this->widget_args["widget_title_link"], 'http://') === FALSE) ? 'http://' . $this->widget_args["widget_title_link"] : $this->widget_args["widget_title_link"];
							
							// Building HTML link atts.
							$linkatts = array('class' => 'srp-widget-title-link', 'href' => $widgetTitleLink);

							// Checking if the "target='_blank'" option is enabled.
							if ($this->widget_args['targetblank_links']  == 'yes') $linkatts['target'] = '_blank';

							// Building widget title HTML.
							$widget_title_link = $this->srp_create_tag('a', $this->srp_sanitize($this->widget_args["widget_title"]), $linkatts);
							...
							$srp_content .= $this->srp_create_tag($this->widget_args['widget_title_header'], $widget_title_link, $widgetTitleAdditionalClasses);
		...
		if (empty($recent_posts)) {
			...
			// Defining total number of posts fetched.
			$post_total_fetched = count($recent_posts);
			...
			foreach($recent_posts as $post) {
				...
				// Calculating odd/even post class.
				$single_post_additional_classes .= ($post_global_counter%2 == 0) ? ' srp-even' : ' srp-odd';
				...
				if ($this->widget_args["post_title_above_thumb"] == 'yes') {
					...
					if ('yes' == $this->widget_args["post_title_nolink"]) {
						...
						// Building HTML link atts.
						$linkatts = array('class' => 'srp-post-title-link', 'href' => get_permalink($post->ID), 'title' => $post->post_title);

						// Checking if the "target='_blank'" option is enabled.
						if ($this->widget_args['targetblank_links']  == 'yes') $linkatts['target'] = '_blank';

						// Building linked post title HTML.
						$ptitlelink  =  $this->srp_create_tag('a', $this->extractTitle($post), $linkatts);
				...
				if ($this->widget_args["display_thumbnail"] == 'yes') {
					...
					switch($this->widget_args["thumbnail_type"]) {
						...
						case "thumb-author":
							...
							$thumb_author_href = ($this->widget_args['author_archive_link'] == 'yes') ? get_author_posts_url(get_the_author_meta('ID')) : get_permalink($post->ID);
							
							// Building HTML link atts.
							$linkatts = array('class' => 'srp-widget-thmblink', 'href' => $thumb_author_href, 'title' => $post->post_title);

							// Checking if the "target='_blank'" option is enabled.
							if ($this->widget_args['targetblank_links']  == 'yes') $linkatts['target'] = '_blank';

							$thumb        = $this->srp_create_tag('a', get_avatar(get_the_author_meta('ID'), $this->widget_args["thumbnail_width"]), $linkatts);
				...
				if ('thumbonly' != $this->widget_args['post_content_mode']) {
					...
					if ('yes' == $this->widget_args["post_date"]) {
						...
						if (!empty($this->widget_args["post_date_prefix"])) {
							$post_date_string .= $this->widget_args["post_date_prefix"];
						}
						...
						$srp_content .= $this->srp_create_tag('p', $post_date_string, array('class' => 'srp-widget-date'));
					}
					...
					if ('yes' == $this->widget_args["post_category"]) {
						...
						if ('yes' == $this->widget_args["post_category_link"]) {
							...
							foreach($srp_categories as $srp_cat) {
								...
								// Building HTML link atts.
								$linkatts = array('href' => $srp_category_link, 'title' => $srp_cat->cat_name);

								// Checking if the "target='_blank'" option is enabled.
								if ($this->widget_args['targetblank_links']  == 'yes') $linkatts['target'] = '_blank';

								// Building category link HTML.						
								$post_cat_list .= $this->srp_create_tag('a', $srp_cat->cat_name, $linkatts) . $this->widget_args["post_category_separator"];
							}
					...
					if ('titleexcerpt' == $this->widget_args["post_content_mode"]) {
						...
						if ($this->widget_args["post_link_excerpt"] == "yes") {
							...
							// Building HTML link atts.
							$linkatts = array('class' => 'srp-linked-excerpt', 'href' => get_permalink($post->ID), 'title' => $post->post_title);

							// Checking if the "target='_blank'" option is enabled.
							if ($this->widget_args['targetblank_links']  == 'yes') $linkatts['target'] = '_blank';

							// Building link tag to enclose the entire excerpt in.
							$srp_content .= $this->srp_create_tag('a', $this->extractContent($post, $this->widget_args["post_content_type"]), $linkatts);
						...
						if ($this->widget_args['image_string_break'] != "") {
							...
							if ('yes' == $this->widget_args['string_break_link']) {
								...
								// Building HTML link atts.
								$linkatts = array('class' => 'srp-widget-stringbreak-link-image', 'href' => get_permalink($post->ID), 'title' => $post->post_title);

								// Checking if the "target='_blank'" option is enabled.
								if ($this->widget_args['targetblank_links']  == 'yes') $linkatts['target'] = '_blank';

								// Building image string break link HTML tag.
								$srp_content .= $this->srp_create_tag('a', $image_string_break, $linkatts);
							...
							if ('yes' == $this->widget_args['string_break_link']) {
								...
								// Building HTML link atts.
								$linkatts = array('class' => 'srp-widget-stringbreak-link', 'href' => get_permalink($post->ID), 'title' => $post->post_title);

								// Checking if the "target='_blank'" option is enabled.
								if ($this->widget_args['targetblank_links']  == 'yes') $linkatts['target'] = '_blank';

								// Building string break link HTML tag.					
								$srp_content .= $this->srp_create_tag('a', $this->srp_sanitize($this->widget_args['string_break']), $linkatts);
					...
					if ('yes' == $this->widget_args["post_tags"]) {
						...
						if (!empty($post_tags)) {
							...
							foreach($post_tags as $tag) {
								...
								$tag_list .= $this->srp_create_tag('a', $tag->name, $linkatts) . $this->widget_args["post_tags_separator"];
							}
				...
				if ($this->widget_args["layout_mode"] == "multi_column") {
					...
					if ( ($post_colrow_counter == $this->widget_args["layout_num_cols"]) || ( ($post_colrow_counter < $this->widget_args["layout_num_cols"]) && ($post_global_counter == $post_total_fetched) ) ) {
